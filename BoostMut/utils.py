...
def add_predictors(df_in, predictor_files, pred_names):
    df_out = df_in.copy()
    # go over each file and match column and indices with df_in
    for file, nam in zip(predictor_files, pred_names):
        # include special exception for FoldX and Rosetta .tab files generated by FRESCO
        if os.path.basename(file) == "MutationsEnergies_CompleteList_FoldX.tab":
            df_score = pd.read_csv(file, skiprows=1,  sep="\s+", index_col=0, names=[nam, 'stdev'])
        elif os.path.basename(file) =="MutationsEnergies_CompleteList_Rosetta.tab":
            df_score = pd.read_csv(file, skiprows=1,  sep="\s+", index_col=0, names=[nam, 'stdev'])
        else:
            df_score = pd.read_csv(file, index_col=0)
        # match rows of df_in and selected column
        df_out[nam] = df_score.loc[df_out.index, nam].values
    return df_out
